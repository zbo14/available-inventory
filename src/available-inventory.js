...
class AvailableInventory extends EventEmitter {
  ...
  run ({incomingOrders, outgoingOrders, shelfLife}) {
    if (!_.isNotEmptyArray(incomingOrders)) {
      this.emit('error', new Error('incomingOrders should be non-empty array'))
    } else if (!_.isNotEmptyArray(outgoingOrders)) {
      this.emit('error', new Error('outgoingOrders sould be non-empty array'))
    } else if (incomingOrders.length !== outgoingOrders.length) {
      this.emit('error', new Error('incomingOrders and outgoingOrders should have same length'))
    } else if (!_.isPositiveNumber(shelfLife)) {
      this.emit('error', new Error('shelfLife should be a positive number'))
    } else {
  ...
  main (products, i = 0, j = 0) {
    if (products[i].isMatch(products[j])) {
      ...
      this.emit('proceed', products, i, j)
    }
  }
